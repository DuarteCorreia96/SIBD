...
drop PROCEDURE IF EXISTS change_units;
...
CREATE PROCEDURE change_units()
BEGIN
UPDATE _indicator j inner join _produced_indicator k ON j.name=k.indicator_name 
SET reference_value=reference_value/10 , units= 'centigrams' ,value=value/10
WHERE units='milligrams';
UPDATE _indicator 
SET reference_value=reference_value/10 , units= 'centigrams'
WHERE units ='milligrams';
END $$
